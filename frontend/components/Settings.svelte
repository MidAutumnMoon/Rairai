<script lang="ts">
    ...
    function credLabel(p: Provider): string {
        if (p.credential.source === "env") {
            return p.credential.ref
                ? `env: ${p.credential.ref}`
                : "env: (no var)";
        }
    ...
    $effect(() => {
        ...
        const desired = models.includes(genModel)
            ? genModel
            : (models[0] ?? "");
    ...
    async function saveProvider() {
        ...
        if (form.credSource === "inline" && !editingId && !form.key) {
            formError =
                "An inline key is required when creating a new provider.";
            return;
        }
...
<div class="settings-content">
    {#if tab === "providers"}
            ...
            <div class="form-card">
                ...
                <div class="grid">
                    ...
                    <div class="field">
                        ...
                        <Select
                            bind:value={form.apiType}
                            items={apiTypeOptions}
                        />
                    </div>
                    <label class="field wide">
                        <span class="lbl">
                            Base URL <span class="hint"
                                >must include /v1 for openai-*</span
                            >
                        </span>
                        <input
                            class="input"
                    ...
                    <div class="field wide">
                        ...
                        {#if form.credSource === "env"}
                            <label class="sub">
                                ...
                                />
                                <span class="hint"
                                    >read from the server environment (not a
                                    secret)</span
                                >
                            </label>
                        {:else}
        ...
        {:else}
            {#each providers as p (p.id)}
                ...
                <div
                    class="prov"
                    class:active={settings?.activeProviderId === p.id}
                >
                    <div class="prov-top">
                        ...
                        {/if}
                        {#if !p.enabled}<span class="tag dim">disabled</span
                            >{/if}
                        <span class="grow"></span>
                        <button
                            class="btn btn-sm"
                    ...
                    <div class="prov-actions">
                        ...
                        <button class="btn btn-sm" onclick={() => testP(p)}
                            >Test</button
                        >
                        {#if p.apiType !== "faux"}
                            <button
    ...
    {:else if tab === "data"}
        <div class="section">
            ...
            <p class="note">
                Provider keys configured as env are read from the server
                environment; inline keys are stored in the server's data dir.
            </p>
        </div>
    {/if}
</div>
